            ...
            for label, unit, kind, series in rows_spec:
                ...
                for i, v in enumerate(vals, start=2):
                    if kind == "date":
                        ...
                        ws.write_number(r, i, float(v) if not is_zero(v) else 0.0, fmt_int)
                    elif kind == "pct":
                        ws.write_number(r, i, float(v) if not is_zero(v) else 0.0, fmt_pct)
                    elif kind == "fx":
                        ws.write_number(r, i, float(v) if not is_zero(v) else 0.0, fmt_fx)
                    elif kind == "brl":
                        mm = float(v) / 1_000_000.0 if not is_zero(v) else 0.0
                        ws.write_number(r, i, mm, fmt_num if "(=)" not in label else fmt_strong)
                    elif kind == "usdmm":
                        mm = float(v) / 1_000_000.0 if not is_zero(v) else 0.0
                        ws.write_number(r, i, mm, fmt_num if "Total Debt Cash Flow" not in label else fmt_strong)
                    else:
